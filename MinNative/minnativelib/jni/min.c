...
jbyteArray Java_com_example_minnative_MinNative_minBuildRawFrameOutput
	(JNIEnv *env, jobject clazz, jint id, jbyteArray data)
{
    uint8_t output[512];
    uint8_t input[255];
    if (data != NULL) {
        memcpy(input, data,  (*env)->GetArrayLength(env, data));
    }
    min_msg_t msg;
    uint32_t len = 0;
    msg.id = id;
    if (data != NULL) {
        msg.len = (*env)->GetArrayLength(env, data);
        msg.payload = input;
    } else {
        msg.len = 0;
        msg.payload = NULL;
...
void Java_com_example_minnative_MinNative_mintest
