...
void min_build_raw_frame_output(min_msg_t *input_msg, uint8_t *output, uint32_t *len)
{
    ...
    *len = 0;
    ...
    char *q = input_msg->payload;
    p = tmp;
    for (int i = 0; i < input_msg->len; i++)
    {
        p += sprintf(p, "%02X", q[i]);
    }
    LOGI("Input : %s", tmp);
...
jbyteArray Java_com_example_gpio_Gpio_minBuildRawFrameOutput
	(JNIEnv *env, jobject clazz, jint id, jbyteArray data)
{
    uint8_t output[512];
    uint8_t input[255];
    min_msg_t msg;
    ...
    if (data != NULL) {
        ...
        char * pCData = (char*)(*env)->GetByteArrayElements(env, data, &isCopy);
        memcpy(input, pCData, size);

        if (isCopy) {
            (*env)->ReleaseByteArrayElements(env, data, pCData, JNI_ABORT);
        }
    ...
    min_build_raw_frame_output(&msg, output, &len);

  	jbyteArray ret = (*env)->NewByteArray(env, len);
  	(*env)->SetByteArrayRegion (env, ret, 0, len, output);
